...
| File | Purpose |
|------|---------|
| `index.html` | **The whole app** — HTML + compiled Tailwind CSS + plain JS: sticky lifeline, search, accordion categories, light/dark mode, **favourites pinning, WhatsApp rows**, service-worker registration — plus a fixed **bottom tab bar** (directory · preparedness checklist · first-aid quick guides · national helplines), each tool tab with its own search, and a small burger menu (copy my location · share this page) |
