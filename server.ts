// Overstrand Lifeline dev server — `bun run dev`
...
if (HOT) {
    try {
        ...
        console.warn(
            "live reload: file watching unavailable —",
            (e as Error).message,
        );
    }
...
console.log(
    `Overstrand Lifeline → http://localhost:${PORT}${HOT ? " (live reload on — HOT=0 to disable)" : ""}`,
);
